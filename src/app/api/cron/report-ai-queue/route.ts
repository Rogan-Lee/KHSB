...
import { listQueuedAiJobs, applyAiJobResult } from "@/lib/report-ai-queue";

// 리포트 AI 텍스트 생성 큐 — 야간 Claude 루틴 연동 엔드포인트.
// 본원 월간 '종합의견' + 멘토링 '코멘트' AI 텍스트를 봉인 프롬프트로 받아 생성.
//   GET  ?limit=30  → 봉인된 프롬프트 N건 반환 (오래 대기한 것부터)
//   POST { results: [{ reportId, markdown }] } → 생성결과 반영 → 타깃 필드 기록
// 인증: Authorization: Bearer ${CRON_SECRET}
// 온라인 큐(/api/cron/online-report-queue)와 동일 계약 — 같은 루틴 프롬프트 재사용 가능.
...
export async function GET(request: NextRequest) {
  ...
  const items = await listQueuedAiJobs(Number.isFinite(limit) ? limit : 30);
...
export async function POST(request: NextRequest) {
  ...
  for (const r of results) {
    ...
    const res = await applyAiJobResult({ jobId: r.reportId, text: r.markdown });
    if (res.ok) applied++;
    else failed++;
