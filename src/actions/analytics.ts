...
export async function getOverallAnalytics(): Promise<OverallAnalytics> {
  ...
  // 같은 /analytics 페이지의 getAttendanceTimeStats 와 동일 범위 (전 원생 성적·재원시간)
...
// 토큰 페이지(/r/[token])·모바일 학부모 리포트는 세션이 없으므로 @/lib/student-analytics 의
// computeStudentAnalytics 를 직접 쓴다. 이 서버 액션은 공개 엔드포인트라 직원만 허용.
export async function getStudentAnalytics(studentId: string): Promise<StudentAnalytics | null> {
  ...
  return computeStudentAnalytics(studentId);
}
