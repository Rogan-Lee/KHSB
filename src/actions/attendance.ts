...
import { requireOwnerOrFullAccess } from "@/lib/roles";
...
export async function createDailyOuting(data: {
  ...
  if (student.mentorId) {
    ...
    requireOwnerOrFullAccess("", session.user.id, session.user.role);
  }
...
export async function updateDailyOuting(id: string, data: {
  ...
  const outing = await prisma.dailyOuting.findUnique({
    where: { id },
    select: { student: { select: { mentorId: true } } },
  });
  if (!outing) throw new Error("외출 기록을 찾을 수 없습니다");
  const mentorId = outing.student.mentorId;
  if (mentorId) {
    requireOwnerOrFullAccess(mentorId, session.user.id, session.user.role);
  } else {
    requireOwnerOrFullAccess("", session.user.id, session.user.role);
  }
